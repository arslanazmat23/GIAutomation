...
export class eventCheckout {
  clickHamburgerEvents(): void {
    ...
    cy.get(
      '.sub-menu > [style="padding-bottom: 7px; padding-top: 7px; border-top: 2px solid rgb(221, 65, 36);"] > a'
    ).click();
  }
  ...
  addAttendees(count: number): void {
    ...
    cy.get('#search-attendees', { timeout: 15000 }).click().clear().type(`${count}`); // Convert count to string and enter the number of attendees
  }
  ...
  fillAttendeeDetails(index: number, attendanceType: string): void {
    ...
    cy.get(
      `#noanim-tab-example-tabpane-0 > .attendee_info > :nth-child(${index + 1}) > .invite_select > .form-group > .form-control`,
      { timeout: 10000 }
  ...
  fillAttendeeDetails1(attendanceType: string): void {
    ...
    cy.get(
      `#noanim-tab-example-tabpane-0 > div > div:nth-child(2) > div.col-xl-6.col-lg-6.col-md-12.invite_select > div > input`,
      { timeout: 10000 }
    )
      .should('be.visible')
      .click({ force: true })
  ...
  fillAttendeeDetails2(attendanceType1: string): void {
    cy.get('.tab-pane.active input[name="attendeefirstname"]').click().type(faker.person.firstName());

    cy.get('.tab-pane.active input[name="attendeelastname"]').click().type(faker.person.lastName());

    cy.get('.tab-pane.active input[name="attendeeemail"]').click().type(faker.internet.email());

    cy.get('.tab-pane.active input[name="attendance"]', { timeout: 10000 });
    cy.get(
      '#noanim-tab-example-tabpane-1 > .attendee_info > :nth-child(2) > .invite_select > .form-group > .file_input_label > .select'
    )
      .click({ force: true })
      .then(() => {
        cy.get(
  ...
  clickProceedToCheckout(): void {
    cy.get('.checkout > .btn', { timeout: 10000 }).should('not.be.disabled').click({ force: true });
  }

  preCheckout(): void {
    cy.get(
      `#noanim-tab-example-tabpane-0 > .attendee_info > :nth-child(1) > :nth-child(1) > .form-group > .form-control`,
      { timeout: 20000 }
    )
      .should('be.visible')
      .invoke('val')
      .then((firstName) => {
        cy.get(
          `#noanim-tab-example-tabpane-0 > .attendee_info > :nth-child(1) > :nth-child(2) > .form-group > .form-control`,
          { timeout: 20000 }
        )
          .should('be.visible')
          .invoke('val')
    ...
    cy.get('body').then(($body) => {
      ...
      cy.get('body').then(($body) => {
        if ($body.text().includes('Please add a credit card to proceed with the checkout.')) {
          ...
          cy.getIframeBody('iframe[name^="__privateStripeFrame"]') // Get iframe body once
            ...
            .then(() => {
              // Use .then() to ensure the type command completes before finding the next element
      ...
      cy.get('body').then(($body) => {
        if ($body.find('.tc > label').length > 0) {
          ...
          cy.get('div.checkout button.btn')
            .should('exist')
            .then(($button) => {
          ...
          cy.get('body').then(($body) => {
            if ($body.find('.checkout > .btn').length > 0 || $body.find('.checkout-btns > :nth-child(2)').length > 0) {
  ...
  fillCheckoutForm_with_Checked_Checkbox(): void {
    ...
    cy.get('@firstName').then(($firstName) => {
      ...
      cy.get('@lastName').then(($lastName) => {
        ...
        cy.get('[name="companyname"]', { timeout: 10000 })
          .clear({ force: true })
          .type(faker.company.name(), { force: true });
  ...
  checkoutReceipt(): void {
    ...
    cy.get('#root > div.main-container > div > div > div.order_data > div > ul > li:nth-child(1) > h5').should(
      'be.visible'
    );
  }
}
